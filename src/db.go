...
func DBAdminInitLoad(idx int) (res string, oerr error) {
  oerr = App.Dao().RunInTransaction(func(txDao *daos.Dao) error {
    ...
    sres, err := txDao.DB().
      NewQuery("SELECT checks.team, checks.prob, checks.id, checks.type, checks.solution, t.name, probs.diff, probs.name FROM checks INNER JOIN teams t ON t.id = checks.team INNER JOIN probs ON probs.id = checks.prob").
      Rows()
      // All(&checkres)
    if err != nil { return err }
