...
func GenProbPaper(dao *daos.Dao) echo.HandlerFunc {
  return func(c echo.Context) error {
    ...
    err = dao.DB().
      NewQuery(`select p.id, p.name, p.diff, p.img, p.author, p.text, p.graph, p.solution, p.infinite, a.username as author_name, a.socials as author_socials from probs as p inner join correctors as a on p.author = a.id where p.contests like concat("%", {:contest}, "%")`).
